    ...
    <div className="bg-black">
      <Marquee className="cursor-pointer" pauseOnHover>
        {effects.map((effect) => (
          <div
            key={effect.id}
            className="mx-2 my-5 rounded-full bg-gradient-to-r from-lime-400 to-cyan-500 px-3 py-1 text-[13px] font-medium shadow-lg transition-transform duration-300 hover:scale-105 dark:text-black md:mx-4"
          >
            {effect.name}
          </div>
        ))}
